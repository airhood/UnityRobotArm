...
python -m model.train --patience 15                 # early stopping patience 설정 (기본값: 10)
...
| 2 | `model/dataset.py` | **image_path 절대경로** — `states.json`에 절대경로로 저장되면 환경 이전 시 전부 깨짐. `collector_server.py`에서 파일명만 저장하도록 수정, `dataset.py`에서 ep_dir 기준으로 resolve. 기존 데이터는 `--fix-paths`로 마이그레이션 | ✅ 해결 |
| 3 | `config.py` | **`TEXT_EMBED_DIM = 384` 잔여 코드** — `all-MiniLM-L6-v2` 시절 설정. 현재 CLIP 사용 기준으로 512가 맞고 실제로도 어디서도 안 씀. 혼란 유발 | 낮음 |
| 4 | `model/dataset.py`, `model/inference.py` | **state 입력 정규화 없음** — joint_angles(degree, ±180 범위)와 ee_position(meter, ±0.3 수준)을 그대로 concat. 스케일 차이로 state encoder 학습 불안정 가능. 대응: joint_angles는 180으로 나눠 [-1,1]로 정규화, ee_position은 workspace 중심/반범위 기준으로 [-1,1] 정규화. inference.py에서도 동일하게 적용 후 역정규화하여 실제 좌표 복원 | ✅ 해결 |
| 5 | `model/dataset.py` | **"return" phase 포함** — 물체 놓고 home으로 돌아가는 구간은 task 완료 후라 유의미한 학습 데이터가 아님. phase 필터링 고려 | 낮음 |
| 6 | `model/architecture.py`, `model/train.py` | **과적합 (overfitting)** — sim2 그룹(2000 ep) 학습 시 epoch 4에서 val loss 최저(0.508), 이후 train loss는 계속 하락하는데 val loss 상승. epoch 62 기준 train=0.295, val=0.650. 원인: 데이터 다양성 부족(동일 scene 반복) + regularization 부족. 대응: (1) Dropout 강화 — state_encoder에 0.2 추가, action_mlp 0.1→0.3으로 증가 (2) Early stopping 추가 — patience epoch 동안 개선 없으면 학습 중단 (기본 patience=10) (3) 근본 해결은 다양한 조건으로 추가 데이터 수집 필요 (목표: 5000~10000 ep) | 진행중 |
